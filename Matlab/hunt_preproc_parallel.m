function hunt_preproc_parallel(subjDir,coregStatsFile,checkRegFile)
...
for i=1:nStudies,
    ...
    if nSeries > 0,
        ...
        if (hasT1_1 == 1 || hasT1_2 == 1),                           
            ...
            studyList(indx,:) = {thisStudy,T1_1File,T1_2File,flairFile,dtiFile,opt};                             
            indx = indx +1;
        end
     end
end

parfor i = 1:length(studyList),    
    pinfo{i}=  doPreproc(studyList{i,1},studyList{i,2},studyList{i,3},studyList{i,4},studyList{i,5},opt);
    writePreprocStamp(studyList{i,1});
end
save(coregStatsFile,'pinfo');
